...
# Model imports
from fileuploadsystem.models import Folder,File
...
def index(request):
    if request.user.is_authenticated:
        ...
        return render(request,'fileuploadsystem/index.html',context)
    else:
...
def folder(request,folderid):
    if request.user.is_authenticated:
        ...
        return render(request,'fileuploadsystem/folder.html',context)
    else:
        return redirect('signup')
# Add Folder View
        ...
        if request.method == 'POST':
            ...
            if username and password and email and cpassword and firstname and lname:
                if password == cpassword:
                    ...
                    redirect("signup")
        return render(request,'fileuploadsystem/signup.html')
        ...
        if request.method == 'POST':
            ...
            if username and password:
                ...
                if user is not None:
                    login(request,user)
                    return redirect('index')
        return render(request,'fileuploadsystem/login.html')
